1: Pergunta 4
Criar uma função (usando ALTER FUNCTION) que retorne a criticidade do cliente com base em quantidade de manutenções e valor gasto, e criar uma query completa que responda: quais são os clientes com maior frequência de manutenções corretivas nos últimos 18 meses, considerando veículos com mais de 3 anos e mecânicos especializados em “Suspensão” ou “Freios”.
A consulta deve retornar: nome, total de veículos, número de corretivas, valor total gasto, tempo médio entre manutenções e o veículo que mais quebrou, além da criticidade gerada pela função.
...
IF (@QtdManutencoes >= 10 OR @ValorTotal >= 10000) SET @Nivel = 'Alta';
ELSE IF (@QtdManutencoes >= 5 OR @ValorTotal >= 5000) SET @Nivel = 'Média';
ELSE SET @Nivel = 'Baixa';
RETURN @Nivel;
END;
GO
ALTER FUNCTION dbo.fn_classifica_criticidade_cliente
...
ManutencoesComIntervalo AS (
SELECT
mc.*,
DATEDIFF(
DAY,
LAG(mc.data_entrada) OVER (PARTITION BY mc.id_cliente ORDER BY mc.data_entrada),
mc.data_entrada
...
COUNT(DISTINCT id_os) AS qtd_manutencoes,
SUM(valor_os) AS valor_total_gasto,
AVG(CAST(dias_desde_anterior AS FLOAT)) AS tempo_medio_entre_manutencoes
FROM ManutencoesComIntervalo
...
mc.id_cliente,
mc.id_veiculo,
COUNT(DISTINCT mcWITH ManutencoesCorretivas AS (
SELECT
c.id_cliente,
c.nome AS nome_cliente,
v.id_veiculo,
...
JOIN servico s ON s.id_servico = osv.id_servico
WHERE
os.data_entrada >= DATEADD(MONTH, -18, CAST(GETDATE() AS DATE))
AND (YEAR(GETDATE()) - v.ano) > 3
AND m.especialidade IN ('Suspensão','Freios')
AND s.descricao LIKE '%corretiv%'
GROUP BY c.id_cliente, c.nome, v.id_veiculo, os.id_os, os.data_entrada
...
ROW_NUMBER() OVER (PARTITION BY mc.id_cliente ORDER BY COUNT(mc.id_os) DESC) AS rn
FROM ManutencoesCorretivas mc
...
ON vmr.id_cliente = t.id_cliente AND vmr.rn = 1
ORDER BY t.qtd_manutencoes DESC, t.valor_total_gasto DESC;
GO
...
A query usa CTEs para separar as etapas:
identificar manutenções corretivas válidas
calcular intervalos entre manutenções
consolidar resumo do cliente
identificar o veículo mais problemático
